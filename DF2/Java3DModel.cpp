/*
 * $Id: Java3DModel.cpp,v 1.33 2008/03/12 16:30:47 matsushima Exp $
 */
 ...
 * @version $Id: Java3DModel.cpp,v 1.33 2008/03/12 16:30:47 matsushima Exp $
 * 
 * $Log: Java3DModel.cpp,v $
 * Revision 1.33  2008/03/12 16:30:47  matsushima
 * Doxygen 対応
 *
 * Revision 1.32  2008/03/06 16:42:45  matsushima
 * JavaDoc/Doxygen 対応
 *
...
/*C*/
#ifdef _IS_H_
class Java3DModel : public Java3D
{
/**/
    /**
     * 予約済みのモデルの種類。
...
#else
    int* Java3DModel::create_color_table(int cnt, const int* t) {
/**/
...
#else
    int* Java3DModel::index_to_color(int* dst, const int* verts, int cnt, int* vert_colors, int mul, int div) {
/**/
        if (null == dst)
        {
            dst = new int[cnt];
...
#else
    int* Java3DModel::zoom_texture(const int* src, int width, int height, int mul_w, int mul_h) {
/**/
...
#else
    int Java3DModel::get_brightness_color(int color, int brightness) {
/**/
        return JavaRenderAbstract/*J*./*C*/::/**/get_brightness_color(color, brightness);
    }
/*C*/
#endif
/**/
...
#else
    int Java3DModel::get_vert_cnt(int* polys, int s) {
/**/
        int i;
...
/**/
     ...
     * 
     * @param   verts   頂点座標配列。
     * @param   index   頂点座標配列内のインデックス。-1 のときは全ての頂点。
...
#else
    void Java3DModel::set_vert(int* verts, int index, int color, int x, int y, int z) {
/**/
        if (-1 == index)
        {
...
#else
    void Java3DModel::set_vert_color(int* verts, int index, int value) {
/**/
        if (-1 == index)
        {
            for (index = 0; EOD != verts[index * 4 + 0]; ++ index)
...
#else
    void Java3DModel::set_vert_xyz(int* verts, int index, int x, int y, int z) {
/**/
        if (-1 == index)
        {
            for (index = 0; EOD != verts[index * 4 + 0]; ++ index)
...
#else
    Java3DModel::Java3DModel(Java3DWorld* world, Java3D* parent) {
/**/
        this->world = world;
        this->parent = parent;
...
/**/
     ...
     * 
     * @param   kind        モデルの種類番号。
     * @param   count       モデルの生存カウント。
     * @param   damage      モデルのダメージ。
     * @param   render_mode モデルの描画モード。
     * @param   verts       モデルの。
     * @param   polys       モデルの。
     * @param   lines       モデルの。
     */
...
/**/
     ...
     */
/*J*
    public void setTexture(JavaRenderTexture texture) {
/*C*/
#ifdef _IS_H_
public:
    virtual void setTexture(JavaRenderTexture* texture);
#else
    void Java3DModel::setTexture(JavaRenderTexture* texture) {
/**/
...
#else
    void Java3DModel::setString(const char* string) {
/**/
        this->string = string;
    }
/*C*/
...
/*J*label_polys:/**/
        for (s = 0; EOD != polys_actual[s]; ) // ポリゴンごと
        {
            ...
            // ポリゴンをポリゴンバッファに追加する
            {
                ...
                trpoly.texture      = (0 != ((SHADING_TEXTURE | RENDER_STRING) & render_actual))
                    ? texture : null;
