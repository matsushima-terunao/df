/*
 * $Id: JavaDF2Models.cpp,v 1.29 2008/04/11 22:52:31 matsushima Exp $
 */
 ...
 * @version $Id: JavaDF2Models.cpp,v 1.29 2008/04/11 22:52:31 matsushima Exp $
 * 
 * $Log: JavaDF2Models.cpp,v $
 * Revision 1.29  2008/04/11 22:52:31  matsushima
 * gound 修正
 *
 * Revision 1.28  2008/03/12 16:30:47  matsushima
 * Doxygen 対応
 *
 * Revision 1.27  2008/03/06 16:42:45  matsushima
 * JavaDoc/Doxygen 対応
 *
 ...
 *
 */
class JavaDF2Models : public JavaDF2ModelsStatic
{
public:
    class Sphere;
    class Ground;
...
/**/
        ...
        /*J*final /**/int color_mark = 0xff00ff00;
        /*J*final /**/int verts_mark[] =
        {
        ...
        /*J*final /**/int polys_mark[] = { 0,1,2,EOD, 3,4,5,EOD, 6,7,8,EOD, 9,10,11,EOD, EOD };
        /*J*final /**/int lines_mark[] = { 0,1,2,0,EOD, 3,4,5,3,EOD, 6,7,8,6,EOD, 9,10,11,9,EOD, EOD };
        model = new Java3DModel(world, null);
        model->init(Java3DModel/*J*./*C*/::/**/KIND_USED, 0, 0, (Java3D::RENDER_DEFAULT & ~mask) | mode,
            verts_mark, polys_mark, lines_mark);
        model_mark = model;
        append(model);

        // Shot
        /*J*final /**/int color_shot = 0xff0000ff;
        /*J*final /**/int verts_shot[] =
        {
            color_shot,-10,0,0, color_shot,-2,1,0, color_shot,-1,1,0,
            color_shot, 10,0,0, color_shot, 1,1,0, color_shot, 2,1,0,
            EOD
        };
        /*J*final /**/int polys_shot[] = { 0,1,2,EOD, 3,4,5,EOD, EOD };
        /*J*final /**/int lines_shot[] = { 0,1,2,0,EOD, 3,4,5,3,EOD, EOD };
        model = new Java3DModel(world, null);
        model->init(Java3DModel/*J*./*C*/::/**/KIND_USED, 0, 0, (Java3D::RENDER_DEFAULT & ~mask) | mode,
...
/**/
        ...
        model_shot->kind = (shot_on ? Java3DModel::KIND_USED : Java3DModel::KIND_UNUSED);
        // Other
/*J*
...
/**/
        ...
        case KIND_ENEMY1:
            {
                /*J*final /**/int test_verts[] = { 0xff00ff00,0,-40,-40, 0xff00ff00,-40,40,-40, 0xff00ff00,40,40,-40, EOD };
                /*J*final /**/int test_polys[] = { 0,1,2,EOD, EOD };
                /*J*final /**/int test_lines[] = { 0,1,2,0,EOD, EOD };
                int r = world->clip_z;
                ...
                int count = r / 16;
                model->init(kind, count, 30, RENDER_DEFAULT, test_verts, test_polys, test_lines);
                model->setPosition(
        ...
        case KIND_ROCK1:
            {
                ...
                int count = r / (16-4);
                i = rand(0, 4);
                model->init(kind, count, 30, RENDER_DEFAULT, _R(rocks[i]).verts, _R(rocks[i]).polys, null);
                model->setTexture(_R(rocks[i]).texture);
                model->setPosition(
...
#else
    void JavaDF2Models::init_fragment2(Java3DModel* model, int kind) {}
    void JavaDF2Models::init_fragment(Java3DModel* model, int kind) {
/**/
        ...
        case KIND_BG_TEST:
            {
                ...
                int vx = 100, vz = 0;
                ///*J*final /**/int verts[] = { C_SHOT|0xff000000,0,-vx,vz, C_SHOT|0x3f000000,-vx,vx,vz, C_SHOT|0x00000000,vx,vx,vz, EOD };
                ///*J*final /**/int polys[] = { 0,1,2, Java3D::EOP, EOD };
                ///*J*final /**/int lines[] = { 0,1,2,0, Java3D::EOP, EOD };
                /*J*final /**/int verts[] = { C_SHOT,-vx,-vx,-vx+vz, C_SHOT,-vx,vx,-vx+vz, C_SHOT,vx,vx,-vx+vz, C_SHOT,vx,-vx,-vx+vz,
                                       C_SHOT,-vx,-vx, vx+vz, C_SHOT,-vx,vx, vx+vz, C_SHOT,vx,vx, vx+vz, C_SHOT,vx,-vx, vx+vz, EOD };
                /*J*final /**/int polys[] = { 0,1,2,3,EOD, 3,2,6,7,EOD, 7,6,5,4,EOD, 4,5,1,0,EOD, 0,3,7,4,EOD, 1,5,6,2,EOD, EOD };
                /*J*final /**/int lines[] = { 0,1,2,0, EOD, EOD };
                ...
                model_bg_test->init(Java3DModel::KIND_USED, 0, 0,
                    Java3D::RENDER_DEFAULT,//(Java3D::RENDER_DEFAULT & ~mask) | mode,
                    verts, polys, lines);
                model_bg_test->mz += 1000;
            }
        case KIND_BG_GROUND:
            {
...
if (0 != DEBUG_VIEW) {
world->setViewport(world->viewport_width / 2, world->viewport_height / 2);
angle = 75;//75;//120;
world->setPerspective(angle, world->max_z, world->clip_z, (int)(world->max_z * 1.0), - (int)(90 * Java3D::ANGLE_MAX / 360));
}
                /*J*final /**/int color_tbl[] =
                {
                        0, 255, 255, 255, // s
                       20, 128, 128, 128, // m
                       40, 100,  30,  10, // m
...
int width = 3000;
model_bg_earth->mz = world->clip_z/1;
model_bg_earth->mb = Java3D::ANGLE_MAX / 6;
model_bg_earth->lx = width;
model_bg_earth->ly = width;
model_bg_earth->lz = -width * 2;
...
#else
        JavaDF2Models::Ground::Ground(Java3DWorld* world, Java3D* parent, int unit_max, const int* color_tbl)
            : Java3DModelGround(world, parent) {
/**/
int dbg_div = 1;
if (0 != DEBUG_VIEW) dbg_div = 8;
            int frac_unit = 8 + 2;
            int frac_width = 1 << frac_unit;
            ...
            int/*J*[]/*C*/* /**/ frac3 = index_to_color(null, frac, frac_width * frac_width, vert_colors, height, INIT_GROUND_HEIGHT);
            //int/*J*[]/*C*/* /**/ frac3 = zoom_texture(frac2, width, width, mul, mul);
            //jdelete(frac2);
            init(world->max_z * 2 / dbg_div, height, INIT_GROUND_HEIGHT, -1, unit_max, Java3D::RENDER_DEFAULT & ~Java3D::SHADING_ALPHA,
                frac, vert_colors);
            models = init_models(UNIT_MIN); // unit: UNIT_MIN .. unit_max
            setTexture(new JavaRenderTexture(frac3, frac_width, frac_width, null, null));
...
/**/
            Java3DModelGround* model = models[unit - UNIT_MIN];
            verts = model->verts;
            polys = model->polys;
            lines = model->lines;
