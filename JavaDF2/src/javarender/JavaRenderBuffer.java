/*
 * $Id: JavaRenderBuffer.java,v 1.4 2008/03/11 16:24:17 matsushima Exp $
 */
 ...
 * @version $Id: JavaRenderBuffer.java,v 1.4 2008/03/11 16:24:17 matsushima Exp $
 * 
 * $Log: JavaRenderBuffer.java,v $
 * Revision 1.4  2008/03/11 16:24:17  matsushima
...
/*C*
#ifdef _IS_H_
class JavaRenderBuffer : public JBase<JavaRenderBuffer>, public JavaRender, public Runnable
{
/**/
/*J*/
    // render フォント
...
#ifdef _IS_H_
public:
    virtual void append_buffer(Polygon* _trpoly, int z_order, int vert_cnt, int mode);
#else
    void JavaRenderBuffer::append_buffer(Polygon* _trpoly, int z_order, int vert_cnt, int mode) {
        JPARAM(Polygon, trpoly);
        JMEMBER(Polygon, trpoly_top);
        JMEMBER(Polygon, trpoly_end);
        JTYPEDEF_P(Polygon);
#define this (*this)
#define _R(p) (*(p))
/**/
++ append_cnt;
        ...
        case APPEND_SORT:
            Polygon i;
            for (i = trpoly_sort_top; trpoly_sort_end != _R(i).next; i = _R(i).next)
            {
++ sort_cnt;
                if (z_order >= _R(_R(i).next).z_order)
                {
                    break;
                }
            }
            trpoly.next = _R(i).next;
            _R(i).next = trpoly;
            break;
        }
        if (null == trpoly.next)
